...
const verifyAccessToken = async (req, res, next) => {
    try {
        ...
        if (!header) return errorResp(res, 401, "Invalid Authentication. (255)")
        if (!header.startsWith("Bearer ")) return errorResp(res, 401, "Invalid Authentication. (256)")

        const token = header.replace("Bearer ", "");
        if (!token) return errorResp(res, 401, "Invalid Authentication. (257)")

        const decoded = jwt.verify(token, process.env.ACCESS_TOKEN_SECRET)
        if (!decoded) return errorResp(res, 401, "Invalid Authentication. (258)")
        if (!decoded.time) return errorResp(res, 401, "Invalid Authentication. (259)")

        const user = await Users.findOne({ _id: decoded.id, accessToken: decoded.time })
        if (!user) return errorResp(res, 401, "Invalid Authentication. (260)")

        req.user = user
        next()
    } catch (err) {
        if (err instanceof jwt.TokenExpiredError) {
            return errorResp(res, 401, "Invalid Authentication. (261)")
        }
